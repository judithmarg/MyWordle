...
def compare_word_first(word_data: WordPlayRequest, db=Depends(get_db)):
    """
...
def compare_word(play_id: int, word_data: WordPlayUpdate, db=Depends(get_db)):
    """
...
def get_all_wordle_attempts(db=Depends(get_db)):
    """
...
def compare_word(play_id: int, db=Depends(get_db)):
    """
